        ...
        if let Some(files) = ctx.input(|input| {
            ...
            for file in files {
                ...
                let content = match read(&file) {
                    Ok(content) => content,
                    Err(error) => {
                        self.errors.buffer.push(error.context("Read file error"));
                        continue;
                    }
                };
        ...
        TopBottomPanel::bottom("bottom_panel").show(ctx, |ui| {
            menu::bar(ui, |ui| {
                ui.menu_button("⚠", |ui| {
                    ui.heading("Errors");
                    ui.separator();
                    self.errors.buffer.retain(|error| {
                        ui.horizontal(|ui| {
                            let text = error.to_string();
                            ui.label(truncate(&text, 80)).on_hover_text(text);
                            // !ui.button("🗙").clicked()
                    ...
                    println!("errors: {}", self.errors.buffer.len());
                });
            });
        });
    }
    ...
    fn left_panel(&mut self, ctx: &Context) {
        SidePanel::left("left_panel").show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if self.normalize {
                    ui.toggle_value(&mut self.percent, "%");
                }
            });
            ui.horizontal(|ui| {
                ui.label("Bound:");
